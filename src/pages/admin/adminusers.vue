<template>
    <v-container>
        <v-data-table v-if="users" :headers="headers" :items="users">
            ...
            <template #item.action="{ item }">
                <v-btn color="primary" icon @click="showUserEdit(item)">
    ...
    <Modal v-model="displayEditModal" title="Edit User" :message="modalMessage" :messageStatus="modalMessageStatus">
        <div class="d-flex justify-center align-center">
            <v-form @submit.prevent="updateUser" class="w-75">
                <v-row>
                    <v-col cols="12" md="12">
                        <v-text-field v-model="user_id" label="User ID" placeholder="User ID" disabled></v-text-field>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col cols="12" md="12">
    ...
    <Modal v-model="displayDeleteModal" title="Delete User" :message="modalMessage" :messageStatus="modalMessageStatus"
        @open="" @close="this.confirmDeleteItem = null">
        <p class="text-center">Are you sure you want to delete user {{ confirmDeleteItem?.username }} with ID of {{
            confirmDeleteItem?.user_id }}?</p>
        <template #actions>
